...
public interface AdminActionRepository extends JpaRepository<AdminAction, UUID> {
  ...
  Page<AdminAction> findByPerformedByOrderByCreatedAtDesc(
      @Param("adminId") UUID adminId, Pageable pageable);

  @Query(
      "SELECT a FROM AdminAction a WHERE a.targetId = :targetId AND a.targetType = :targetType"
          + " ORDER BY a.createdAt DESC")
  Page<AdminAction> findByTargetIdAndTargetTypeOrderByCreatedAtDesc(
      @Param("targetId") UUID targetId, @Param("targetType") String targetType, Pageable pageable);

  @Query("SELECT a FROM AdminAction a WHERE a.isActive = true ORDER BY a.createdAt DESC")
  Page<AdminAction> findByIsActiveOrderByCreatedAtDesc(
      @Param("isActive") boolean isActive, Pageable pageable);

  @Query("SELECT a FROM AdminAction a WHERE a.type = :type ORDER BY a.createdAt DESC")
  Page<AdminAction> findByTypeOrderByCreatedAtDesc(
      @Param("type") AdminAction.AdminActionType type, Pageable pageable);
  ...
  @Query(
      "SELECT COUNT(a) FROM AdminAction a WHERE a.performedBy = :adminId AND a.createdAt >= :since")
  ...
  long countActionsByTypeSince(
      @Param("type") AdminAction.AdminActionType type, @Param("since") Instant since);
  ...
  @Query(
      "SELECT a.performedBy, COUNT(a) FROM AdminAction a WHERE a.createdAt >= :since GROUP BY"
          + " a.performedBy ORDER BY COUNT(a) DESC")
