...
public interface FileUploadRepository extends JpaRepository<FileUpload, UUID> {
  ...
  Page<FileUpload> findByUploadedByOrderByCreatedAtDesc(
      @Param("userId") UUID userId, Pageable pageable);

  @Query(
      "SELECT f FROM FileUpload f WHERE f.type = :type AND f.isPublic = :isPublic ORDER BY"
          + " f.uploadedAt DESC")
  Page<FileUpload> findByTypeAndIsPublicOrderByCreatedAtDesc(
      @Param("type") FileUpload.FileUploadType type,
      @Param("isPublic") boolean isPublic,
      Pageable pageable);

  @Query(
      "SELECT f FROM FileUpload f WHERE f.uploadedBy.id = :userId AND f.type = :type ORDER BY"
          + " f.uploadedAt DESC")
  Page<FileUpload> findByUploadedByAndTypeOrderByCreatedAtDesc(
  ...
  @Query(
      "SELECT f FROM FileUpload f WHERE f.isNSFW = :isNSFW AND f.isPublic = true ORDER BY"
          + " f.uploadedAt DESC")
  Page<FileUpload> findPublicNSFWFilesOrderByCreatedAtDesc(
      @Param("isNSFW") boolean isNSFW, Pageable pageable);

  @Query(
      "SELECT f FROM FileUpload f WHERE f.uploadedBy.id = :userId AND f.isPublic = :isPublic ORDER"
          + " BY f.uploadedAt DESC")
  Page<FileUpload> findByUploadedByAndIsPublicOrderByCreatedAtDesc(
      @Param("userId") UUID userId, @Param("isPublic") boolean isPublic, Pageable pageable);
  ...
  long countFilesByUserAndType(
      @Param("userId") UUID userId, @Param("type") FileUpload.FileUploadType type);
  ...
  @Query(
      "SELECT f.type, COUNT(f) FROM FileUpload f WHERE f.uploadedBy.id = :userId GROUP BY f.type")
  List<Object[]> getFileTypeStatsByUser(@Param("userId") UUID userId);

  @Query(
      "SELECT f.uploadedBy.id, COUNT(f) FROM FileUpload f GROUP BY f.uploadedBy.id ORDER BY"
          + " COUNT(f) DESC")
