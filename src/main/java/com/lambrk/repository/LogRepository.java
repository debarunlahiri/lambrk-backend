...
public interface LogRepository extends JpaRepository<LogEntry, UUID> {
  ...
  Page<LogEntry> findByEndpointContainingIgnoreCaseOrderByTimestampDesc(
  ...
  @Query(
      "SELECT l FROM LogEntry l WHERE l.timestamp BETWEEN :start AND :end ORDER BY l.timestamp"
          + " DESC")
  Page<LogEntry> findByTimestampBetween(
      @Param("start") Instant start, @Param("end") Instant end, Pageable pageable);

  @Query(
      "SELECT l FROM LogEntry l WHERE l.userId = :userId AND l.timestamp BETWEEN :start AND :end"
          + " ORDER BY l.timestamp DESC")
  Page<LogEntry> findByUserIdAndTimestampBetween(
  ...
  @Query(
      "SELECT l FROM LogEntry l WHERE l.endpoint LIKE %:endpoint% AND l.timestamp BETWEEN :start"
          + " AND :end ORDER BY l.timestamp DESC")
  Page<LogEntry> findByEndpointContainingAndTimestampBetween(
  ...
  @Query(
      "SELECT l.endpoint, COUNT(l) FROM LogEntry l WHERE l.timestamp >= :since GROUP BY l.endpoint"
