...
@Repository
public interface UserCommunityMembershipRepository
    extends JpaRepository<UserCommunityMembership, UUID> {
  ...
  List<UserCommunityMembership> findByCommunityIdAndStatus(
      UUID communityId, MembershipStatus status);

  boolean existsByUserIdAndCommunityIdAndStatus(
      UUID userId, UUID communityId, MembershipStatus status);
