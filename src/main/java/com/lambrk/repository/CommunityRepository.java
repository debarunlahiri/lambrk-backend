...
public interface CommunityRepository
    extends JpaRepository<Community, UUID>, JpaSpecificationExecutor<Community> {
  ...
  @Query(
      "SELECT s FROM Community s WHERE s.isPublic = true AND s.createdAt >= :since ORDER BY"
  ...
  @Query(
      "UPDATE Community s SET s.subscriberCount = s.subscriberCount + :delta WHERE s.id ="
          + " :communityId")
  @Modifying
  void updateSubscriberCount(@Param("communityId") UUID communityId, @Param("delta") int delta);

  @Query(
      "UPDATE Community s SET s.activeUserCount = s.activeUserCount + :delta WHERE s.id ="
          + " :communityId")
  @Modifying
  ...
  @Query(
      "SELECT s FROM Community s WHERE s.subscriberCount >= :minSubscribers AND s.isPublic = true"
          + " ORDER BY s.subscriberCount DESC")
  Page<Community> findCommunitiesByMinSubscribers(
      @Param("minSubscribers") int minSubscribers, Pageable pageable);

  @Query(
      "SELECT s FROM Community s WHERE s.activeUserCount >= :minActiveUsers AND s.isPublic = true"
          + " ORDER BY s.activeUserCount DESC")
  Page<Community> findCommunitiesByMinActiveUsers(
      @Param("minActiveUsers") int minActiveUsers, Pageable pageable);

  @Query(
      "SELECT s FROM Community s JOIN s.memberships m WHERE m.user.id = :userId AND m.status ="
          + " 'ACTIVE'")
  Set<Community> findSubscribedCommunitiesByUser(@Param("userId") UUID userId);

  @Query(
      "SELECT s FROM Community s JOIN s.moderators m WHERE m.user.id = :userId AND m.isActive ="
          + " true")
  ...
  @Query(
      "SELECT s FROM Community s WHERE s.isPublic = true AND (s.name LIKE %:query% OR s.title LIKE"
          + " %:query% OR s.description LIKE %:query%)")
  ...
  @Query(
      "SELECT s FROM Community s WHERE s.isPublic = true AND s.activeUserCount > 0 ORDER BY"
          + " s.activeUserCount DESC")
