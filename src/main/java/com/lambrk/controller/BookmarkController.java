...
public class BookmarkController {
  ...
  @Counted(
      value = "bookmarks.created",
      extraTags = {"action", "bookmark"})
  ...
  @Counted(
      value = "bookmarks.deleted",
      extraTags = {"action", "unbookmark"})
    ...
    Page<PostResponse> response =
        posts.map(
            post ->
                PostResponse.from(
                    post,
                    postService.getUserVote(post, userId),
                    bookmarkService.isBookmarked(userId, post.getId())));
