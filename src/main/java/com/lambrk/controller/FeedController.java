...
public class FeedController {
  ...
  @Operation(
      ...
      description =
          "Returns a personalized feed of posts and suggested users. Authenticated users get"
              + " personalized ranking; anonymous users get trending content.")
  @ApiResponses(
      value = {
  ...
  public ResponseEntity<FeedResponse> getFeed(
      ...
      @Parameter(description = "Sort method: algorithm, hot, new, top (default: algorithm)")
          @RequestParam(defaultValue = "algorithm")
          String sortBy,
  ...
  @Operation(
      ...
      description =
          "Returns a personalized feed with advanced filtering options for post types and content"
              + " preferences.")
  @ApiResponses(
      value = {
        @ApiResponse(responseCode = "200", description = "Feed generated successfully"),
  ...
  public ResponseEntity<FeedResponse> getFeedWithFilters(
      @AuthenticationPrincipal UserPrincipal userDetails, @Valid @RequestBody FeedRequest request) {
    UUID userId = getUserIdFromUserDetails(userDetails);
    logger.info("Generating personalized feed with filters for user: {}", userId);

    // Override the userId from the authenticated user
    FeedRequest authenticatedRequest =
        new FeedRequest(
            userId,
            request.limit(),
            request.sortBy(),
  ...
  public ResponseEntity<FeedResponse> getHotFeed(
    ...
    FeedRequest request =
        new FeedRequest(
            userId, limit, "hot", null, false, false, 2.0 // Faster time decay for hot content
            );
  ...
  public ResponseEntity<FeedResponse> getNewFeed(
    ...
    FeedRequest request =
        new FeedRequest(
            userId, limit, "new", null, false, false, 0.1 // Minimal time decay for newest first
            );
  ...
  public ResponseEntity<FeedResponse> getTopFeed(
    ...
    UUID userId = getUserIdFromUserDetails(userDetails);
    logger.info(
        "Generating top feed for user: {} with limit: {} and timePeriod: {}",
        userId,
        limit,
        timePeriod);

    FeedRequest request = new FeedRequest(userId, limit, "top", null, false, false, 0.5);
  ...
  @Operation(
      ...
      description =
          "Returns posts from communities the user doesn't follow to discover new content.")
  public ResponseEntity<FeedResponse> getDiscoverFeed(
    ...
    FeedRequest request = new FeedRequest(userId, limit, "discover", null, false, false, 1.5);
