...
public class RecommendationService {
  ...
  @Cacheable(
      ...
      key =
          "#request.userId() + '-' + #request.type() + '-' + #request.limit() + '-' +"
              + " (#currentUserId == null ? 'anonymous' : #currentUserId)")
  @CircuitBreaker(name = "userService")
  @Retry(name = "userService")
  public RecommendationResponse getRecommendations(
      RecommendationRequest request, UUID currentUserId) {
    try {
      User user =
          userRepository
      ...
      return switch (request.type()) {
        ...
        case USERS -> getUserRecommendations(user, userHistory, request, currentUserId);
  ...
  private RecommendationResponse getPostRecommendations(
      User user, List<Post> userHistory, RecommendationRequest request) {
    try {
      ...
      List<Post> recommendations =
          combinePostRecommendations(similarPosts, trendingPosts, personalized, request);
  ...
  private RecommendationResponse getCommunityRecommendations(
      User user, List<Post> userHistory, RecommendationRequest request) {
    try {
      ...
      List<Community> recommendations =
          combineCommunityRecommendations(userCommunities, similar, trending, request);
  ...
  private RecommendationResponse getUserRecommendations(
      User user, List<Post> userHistory, RecommendationRequest request, UUID currentUserId) {
    try {
      ...
      return RecommendationResponse.ofUsers(
          recommendations.stream()
              .map(
                  recommendedUser ->
                      userSocialService.toSocialUserResponse(recommendedUser, currentUserId))
              .toList(),
          explanation,
          confidence,
          factors);
  ...
  private RecommendationResponse getCommentRecommendations(
      User user, List<Post> userHistory, RecommendationRequest request) {
    try {
      // For comments, we'll return a simplified response since we don't have comment
      // recommendations yet
      String explanation = "Recommended comments based on your activity and interests";
      double confidence = 0.7;
      List<String> factors =
          List.of("User activity patterns", "Community engagement", "Comment quality");
  ...
  private List<Post> findTrendingPosts(User user, RecommendationRequest request) {
    List<Community> userCommunities = getUserCommunityPreferences(user);
    List<String> communityNames = userCommunities.stream().map(Community::getName).toList();

    Pageable pageable = PageRequest.of(0, request.limit(), Sort.by(Sort.Direction.DESC, "score"));
    return postRepository.findAll(pageable).getContent();
  }

  private List<Post> getAIRecommendations(
      User user, List<Post> userHistory, String type, RecommendationRequest request) {
    if (userHistory.isEmpty()) {
      return List.of();
    }
    ...
    try {
      String prompt =
          String.format(
              ...
              """,
              formatUserHistory(userHistory), request.limit());
  ...
  private List<Post> combinePostRecommendations(
      List<Post> similar,
      List<Post> trending,
  ...
  private List<Community> combineCommunityRecommendations(
  ...
  private List<User> combineUserRecommendations(
      List<User> similar, List<User> active, RecommendationRequest request) {
  ...
  private String generateExplanation(User user, String type, int count) {
    return String.format(
        "Based on your activity in %d communities and %d interactions, "
