...
public class UserSocialService {
  ...
  public void follow(UUID followerId, UUID followingId, String source) {
    ...
    UserFollow follow =
        existingFollow.orElseGet(
            () -> new UserFollow(UuidV7Generator.generate(), follower, following, source));
  ...
  public UserFriendship sendFriendRequest(
      UUID requesterId, UUID addresseeId, FriendRequestCreateRequest request) {
    ...
    Optional<UserFriendship> existing =
        userFriendshipRepository.findByUserOneAndUserTwo(
            orderedUsers.userOne(), orderedUsers.userTwo());
  ...
  public Page<SocialUserResponse> getFollowers(UUID userId, UUID currentUserId, Pageable pageable) {
    ...
    return filterHiddenMutualUsers(
            userFollowRepository.findFollowers(user, pageable), currentUserId, pageable)
  ...
  public Page<SocialUserResponse> getFollowing(UUID userId, UUID currentUserId, Pageable pageable) {
    ...
    return filterHiddenMutualUsers(
            userFollowRepository.findFollowing(user, pageable), currentUserId, pageable)
  ...
  @Transactional(readOnly = true)
  public Page<SocialUserResponse> getMutualFollowers(
      UUID userId, UUID withUserId, UUID currentUserId, Pageable pageable) {
    User user = getUser(userId);
    User withUser = getUser(withUserId);
    if (!canViewFollowerList(user, currentUserId)
        || !canViewFollowerList(withUser, currentUserId)) {
      return Page.empty(pageable);
    }
    return filterHiddenMutualUsers(
            userFollowRepository.findMutualFollowers(user, withUser, pageable),
            currentUserId,
            pageable)
  ...
  @Transactional(readOnly = true)
  public Page<SocialUserResponse> getMutualFollowing(
      UUID userId, UUID withUserId, UUID currentUserId, Pageable pageable) {
    User user = getUser(userId);
    User withUser = getUser(withUserId);
    if (!canViewFollowingList(user, currentUserId)
        || !canViewFollowingList(withUser, currentUserId)) {
      return Page.empty(pageable);
    }
    return filterHiddenMutualUsers(
            userFollowRepository.findMutualFollowing(user, withUser, pageable),
            currentUserId,
            pageable)
        .map(mutualUser -> toSocialUserResponse(mutualUser, currentUserId));
  }
  ...
  @Transactional(readOnly = true)
  public Page<SocialUserResponse> getMutualFriends(
      UUID userId, UUID withUserId, UUID currentUserId, Pageable pageable) {
    User user = getUser(userId);
    User withUser = getUser(withUserId);
    return filterHiddenMutualUsers(
            userFriendshipRepository.findMutualFriends(user, withUser, pageable),
            currentUserId,
            pageable)
        .map(mutualUser -> toSocialUserResponse(mutualUser, currentUserId));
  }
  ...
  @Transactional(readOnly = true)
  public Page<FriendRequestResponse> getIncomingFriendRequests(
      UUID currentUserId, Pageable pageable) {
    User user = getUser(currentUserId);
    return userFriendshipRepository
        .findIncomingRequests(user, pageable)
        .map(FriendRequestResponse::from);
  }

  @Transactional(readOnly = true)
  public Page<FriendRequestResponse> getOutgoingFriendRequests(
      UUID currentUserId, Pageable pageable) {
    User user = getUser(currentUserId);
    return userFriendshipRepository
        .findOutgoingRequests(user, pageable)
        .map(FriendRequestResponse::from);
  }
  ...
  public SocialUserResponse toSocialUserResponse(User user, UUID currentUserId) {
    ...
    if (currentUserId != null) {
      ...
      followedByCurrentUser =
          userFollowRepository.existsByFollowerAndFollowingAndStatus(
              currentUser, user, UserFollow.FollowStatus.ACTIVE);
      followingCurrentUser =
          userFollowRepository.existsByFollowerAndFollowingAndStatus(
              user, currentUser, UserFollow.FollowStatus.ACTIVE);
  ...
  public User updatePrivacySettings(
      UUID userId, com.lambrk.dto.UserPrivacySettingsRequest request) {
    User user = getUser(userId);
    if (request.privateAccount() != null) {
      applyPrivateAccountPreset(user, request.privateAccount());
    } else {
      if (request.hideFollowerCount() != null)
        user.setHideFollowerCount(request.hideFollowerCount());
      if (request.hideFollowingCount() != null)
        user.setHideFollowingCount(request.hideFollowingCount());
      if (request.hideFollowerList() != null) user.setHideFollowerList(request.hideFollowerList());
      if (request.hideFollowingList() != null)
        user.setHideFollowingList(request.hideFollowingList());
      if (request.hideAddFriendButton() != null)
        user.setHideAddFriendButton(request.hideAddFriendButton());
      if (request.hideFollowButton() != null) user.setHideFollowButton(request.hideFollowButton());
      if (request.hideFromMutualList() != null)
        user.setHideFromMutualList(request.hideFromMutualList());
      if (request.messageButtonEnabled() != null)
        user.setMessageButtonEnabled(request.messageButtonEnabled());
    }
        ...
        .orElseThrow(
            () ->
                new ResourceNotFoundException(
                    "Friendship", "users", first.getId() + "," + second.getId()));
  }

  private Optional<UserFriendship> findFriendship(User first, User second) {
    OrderedUsers orderedUsers = orderUsers(first, second);
    return userFriendshipRepository.findByUserOneAndUserTwo(
        orderedUsers.userOne(), orderedUsers.userTwo());
  }

  private boolean canViewFollowerCount(
      User user, UUID currentUserId, boolean friend, boolean owner) {
    return canViewPrivateAccount(user, currentUserId, friend, owner)
        && (owner || !user.isHideFollowerCount());
  }

  private boolean canViewFollowingCount(
      User user, UUID currentUserId, boolean friend, boolean owner) {
    return canViewPrivateAccount(user, currentUserId, friend, owner)
        && (owner || !user.isHideFollowingCount());
  }
  ...
  private boolean canViewFollowerList(
      User user, UUID currentUserId, boolean friend, boolean owner) {
    return canViewPrivateAccount(user, currentUserId, friend, owner)
        && (owner || !user.isHideFollowerList());
  }
  ...
  private boolean canViewFollowingList(
      User user, UUID currentUserId, boolean friend, boolean owner) {
    return canViewPrivateAccount(user, currentUserId, friend, owner)
        && (owner || !user.isHideFollowingList());
  }

  private boolean canShowAddFriendButton(
      User user, UUID currentUserId, boolean friend, boolean owner) {
    return !owner
        && !friend
        && canViewPrivateAccount(user, currentUserId, friend, owner)
  ...
  private Page<User> filterHiddenMutualUsers(
      Page<User> users, UUID currentUserId, Pageable pageable) {
  ...
  private boolean canViewPrivateAccount(
      User user, UUID currentUserId, boolean friend, boolean owner) {
