...
public class FreeTierLimitService {
  ...
  public void checkUploadAllowed(UUID userId, long fileSizeBytes) {
    ...
    Optional<FreeTierUsage> usageOpt =
        freeTierUsageRepository.findFreeTierUsageByUserAndPeriod(
            userId, currentMonth.getYear(), currentMonth.getMonthValue());

    FreeTierUsage usage = usageOpt.orElseGet(() -> FreeTierUsage.createForUser(userId, true));
    ...
    if (projectedStorage > storageLimitBytes) {
      throw new FreeTierLimitExceededException(
          "Storage limit exceeded. Free tier limit: "
              + storageLimitMB
              + "MB. "
              + "Current usage: "
              + (usage.getStorageBytesUsed() / 1024 / 1024)
              + "MB, "
              + "File size: "
    ...
    if (usage.getUploadsCount() >= monthlyUploadLimit) {
      throw new FreeTierLimitExceededException(
          "Monthly upload limit exceeded. Free tier limit: "
              + monthlyUploadLimit
              + " uploads/month. "
              + "Current uploads: "
  ...
  public FreeTierUsage recordUpload(UUID userId, long fileSizeBytes) {
    ...
    YearMonth currentMonth = YearMonth.now();
    Optional<FreeTierUsage> usageOpt =
        freeTierUsageRepository.findByUserIdAndPeriodYearAndPeriodMonth(
            userId, currentMonth.getYear(), currentMonth.getMonthValue());

    FreeTierUsage usage = usageOpt.orElseGet(() -> FreeTierUsage.createForUser(userId, true));

    FreeTierUsage updated = usage.withAddedStorage(fileSizeBytes).withIncrementedUploads();

    FreeTierUsage saved = freeTierUsageRepository.save(updated);
    logger.info(
        "Recorded upload for user {}: {} bytes. Total storage: {} bytes, Uploads: {}",
        userId,
        fileSizeBytes,
        saved.getStorageBytesUsed(),
        saved.getUploadsCount());
  ...
  public void recordBandwidthUsage(UUID userId, long bytesTransferred) {
    ...
    if (projectedBandwidth > bandwidthLimitBytes) {
      throw new FreeTierLimitExceededException(
          "Monthly bandwidth limit exceeded. Free tier limit: "
              + monthlyBandwidthGB
              + "GB/month. "
              + "Current bandwidth: "
              + (usage.getBandwidthBytes() / 1024 / 1024 / 1024)
              + "GB");
    }
  ...
  public FreeTierStatus getUserStatus(UUID userId) {
    ...
    boolean withinLimits =
        usage.getStorageBytesUsed() < storageLimitBytes
