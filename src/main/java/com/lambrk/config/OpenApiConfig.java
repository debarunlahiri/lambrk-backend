...
public class OpenApiConfig {
    ...
    return new OpenAPI()
        .info(
            new Info()
                ...
                .version("1.0.0")
                .description(
                    "Production-grade Lambrk-like backend API with Spring Boot 3.5 and Java 25")
                .contact(new Contact().name("API Support").email("api@lambrk.com"))
                .license(
  ...
  public GroupedOpenApi postsApi() {
    return GroupedOpenApi.builder().group("posts").pathsToMatch("/api/posts/**").build();
  }

  @Bean
  public GroupedOpenApi commentsApi() {
    return GroupedOpenApi.builder().group("comments").pathsToMatch("/api/comments/**").build();
  }
  ...
  public GroupedOpenApi votesApi() {
    return GroupedOpenApi.builder().group("votes").pathsToMatch("/api/votes/**").build();
  }

  @Bean
  public GroupedOpenApi searchApi() {
    return GroupedOpenApi.builder().group("search").pathsToMatch("/api/search/**").build();
  }
  ...
  public GroupedOpenApi filesApi() {
    return GroupedOpenApi.builder().group("files").pathsToMatch("/api/files/**").build();
  }
