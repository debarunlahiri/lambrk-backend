...
class NotificationIntegrationTest {
    ...
    mockMvc
        .perform(
            post("/api/notifications")
                ...
                .content(
                    "{\"type\":\"COMMENT_REPLY\",\"recipientId\":1,\"title\":\"New reply to your"
                        + " comment\",\"message\":\"Someone replied to your"
                        + " comment\",\"relatedPostId\":1,\"relatedCommentId\":10,\"actionUrl\":\"/posts/1#comment-10\",\"actionText\":\"View"
                        + " reply\",\"isRead\":false}"))
  ...
  void shouldGetUnreadNotifications() throws Exception {
    mockMvc
        .perform(get("/api/notifications/unread").param("page", "0").param("size", "10"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.content").isArray());
  }
  ...
  void shouldMarkNotificationAsRead() throws Exception {
    mockMvc.perform(put("/api/notifications/1/read")).andExpect(status().isOk());
  }
  ...
  void shouldDeleteAllNotifications() throws Exception {
    mockMvc.perform(delete("/api/notifications")).andExpect(status().isNoContent());
  }

  @Test
  void shouldGetUnreadCount() throws Exception {
    mockMvc
        .perform(get("/api/notifications/count/unread"))
        .andExpect(status().isOk())
    ...
    mockMvc
        .perform(
            post("/api/notifications")
                .contentType("application/json")
                .content(
                    "{\"type\":\"INVALID\",\"recipientId\":null,\"title\":\"\",\"message\":\"\"}"))
