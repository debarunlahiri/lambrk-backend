...
class AdminIntegrationTest {
    ...
    mockMvc
        .perform(
            post("/api/admin/actions")
                ...
                .content(
                    "{\"type\":\"DELETE_POST\",\"targetId\":1,\"reason\":\"Violation of community"
                        + " guidelines\",\"notes\":\"Spam"
                        + " content\",\"durationDays\":null,\"permanent\":false,\"notifyUser\":true}"))
  ...
  void shouldDeleteComment() throws Exception {
    mockMvc
        .perform(
            post("/api/admin/delete-comment/1")
  ...
  void shouldQuarantinePost() throws Exception {
    mockMvc
        .perform(
            post("/api/admin/quarantine-post/1")
  ...
  void shouldGetAdminActions() throws Exception {
    mockMvc
        .perform(get("/api/admin/actions").param("page", "0").param("size", "10"))
        .andExpect(status().isOk())
  ...
  void shouldGetAdminActionsByUser() throws Exception {
    mockMvc
        .perform(get("/api/admin/actions/user/1").param("page", "0").param("size", "10"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.content").isArray());
  }
  ...
  void shouldGetActiveActions() throws Exception {
    mockMvc
        .perform(get("/api/admin/actions/active").param("page", "0").param("size", "10"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.content").isArray());
  }
