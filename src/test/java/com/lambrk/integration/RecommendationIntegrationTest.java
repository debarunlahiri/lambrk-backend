...
class RecommendationIntegrationTest {
  ...
  void shouldGetCommunityRecommendations() throws Exception {
    mockMvc
        .perform(
            get("/api/recommendations/communities/1")
                .param("limit", "10")
                .param("includeNSFW", "false")
                .param("includeOver18", "false"))
        .andExpect(status().isOk())
  ...
  void shouldGetUserRecommendations() throws Exception {
    mockMvc
        .perform(get("/api/recommendations/users/1").param("limit", "10"))
        .andExpect(status().isOk())
  ...
  void shouldGetCommentRecommendations() throws Exception {
    mockMvc
        .perform(get("/api/recommendations/comments/1").param("limit", "10"))
        .andExpect(status().isOk())
  ...
  void shouldGetContextualRecommendations() throws Exception {
    mockMvc
        .perform(
            get("/api/recommendations/context/1")
  ...
  void shouldGetTrendingRecommendations() throws Exception {
    mockMvc
        .perform(get("/api/recommendations/trending").param("type", "posts").param("limit", "10"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.type").value("POSTS"))
        .andExpect(jsonPath("$.posts").isArray());
  }
    ...
    mockMvc
        .perform(
            post("/api/recommendations")
                ...
                .content(
                    "{\"userId\":1,\"type\":\"POSTS\",\"limit\":10,\"excludeCommunities\":[],\"excludeUsers\":[],\"includeNSFW\":false,\"includeOver18\":false,\"contextCommunityId\":null,\"contextPostId\":null}"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.type").value("POSTS"))
        .andExpect(jsonPath("$.posts").isArray());
  }
