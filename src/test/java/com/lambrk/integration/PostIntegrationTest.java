...
class PostIntegrationTest {
  ...
  void shouldGetHotPosts() throws Exception {
    mockMvc
        .perform(get("/api/posts/hot").param("page", "0").param("size", "10"))
  ...
  void shouldGetNewPosts() throws Exception {
    mockMvc
        .perform(get("/api/posts/new").param("page", "0").param("size", "10"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.content").isArray());
  }
  ...
  void shouldGetTopPosts() throws Exception {
    mockMvc
        .perform(get("/api/posts/top").param("page", "0").param("size", "10"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.content").isArray());
  }
  ...
  void shouldReturn401ForUnauthenticatedRequests() throws Exception {
    mockMvc.perform(get("/api/posts/hot")).andExpect(status().isUnauthorized());
  }
