...
class SearchIntegrationTest {
    ...
    mockMvc
        .perform(
            post("/api/search")
                ...
                .content(
                    "{\"query\":\"spring"
                        + " boot\",\"type\":\"ALL\",\"sort\":\"RELEVANCE\",\"timeFilter\":\"ALL\",\"page\":0,\"size\":20}"))
  ...
  void shouldSearchPostsOnly() throws Exception {
    mockMvc
        .perform(
            get("/api/search/posts").param("query", "java").param("page", "0").param("size", "10"))
        .andExpect(status().isOk())
  ...
  void shouldSearchUsers() throws Exception {
    mockMvc
        .perform(
            get("/api/search/users").param("query", "john").param("page", "0").param("size", "10"))
        .andExpect(status().isOk())
  ...
  void shouldGetSearchSuggestions() throws Exception {
    mockMvc
        .perform(get("/api/search/suggestions").param("query", "spring").param("type", "posts"))
        .andExpect(status().isOk())
  ...
  void shouldGetTrendingSearches() throws Exception {
    mockMvc
        .perform(get("/api/search/trending").param("page", "0").param("size", "10"))
        .andExpect(status().isOk())
