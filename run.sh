...
verify_tables() {
    ...
    for stale in "${STALE_TABLES[@]}"; do
        if echo "$EXISTING_TABLES" | grep -q "^${stale}$"; then
            HAS_STALE=true
            warn "Found stale table '$stale' (pre-rename)"
        fi
    done

    # Check if flyway_schema_history is missing but other tables exist
    HAS_HISTORY=$(echo "$EXISTING_TABLES" | grep -c "^flyway_schema_history$" || true)
    NEEDS_RESET=false

    if [ "$HAS_STALE" = true ]; then
        warn "Database has stale tables from before rename. Resetting database..."
        NEEDS_RESET=true
    elif [ "$HAS_HISTORY" -eq 0 ] && [ -n "$EXISTING_TABLES" ]; then
        warn "flyway_schema_history missing but tables exist. Resetting database..."
        NEEDS_RESET=true
    fi
    ...
    if [ ${#MISSING_TABLES[@]} -gt 0 ]; then
        ...
        if $MVN_CMD flyway:migrate -q 2>&1 | tail -10; then
            success "Flyway migrations completed"
            ...
            for table in "${MISSING_TABLES[@]}"; do
                ...
                if [ "$EXISTS" = "t" ]; then
                    ...
                    fail "Table '$table' is still missing after bootstrap/migration"
                    exit 1
                fi
            done
        else
            fail "Flyway migrations failed. Run manually: ./mvnw flyway:migrate"
            exit 1
        fi
    else
