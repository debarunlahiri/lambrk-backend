...
set MISSING_TABLES=0
for %%t in (users sublambrks posts comments votes user_sublambrk_memberships user_sublambrk_moderators free_tier_usage api_logs) do (
    docker exec lambrk-postgres psql -U %DB_USER% -d %DB_NAME% -t -c "SELECT 1 FROM %%t LIMIT 1;" >nul 2>&1
    if !errorlevel!==0 (
...
if !MISSING_TABLES!==1 (
    echo.
    echo   [WARN] Missing tables detected. Running DB bootstrap + Flyway migrations...

    if exist "%PROJECT_DIR%scripts\bootstrap-db.bat" (
        call "%PROJECT_DIR%scripts\bootstrap-db.bat"
        if !errorlevel! neq 0 (
            echo   [WARN] DB bootstrap script failed; continuing with Flyway
        ) else (
            echo   [OK] DB bootstrap SQL completed
        )
    )

    if exist "%PROJECT_DIR%mvnw.cmd" (
        call mvnw.cmd flyway:migrate -q 2>nul
    ) else (
        call mvn flyway:migrate -q 2>nul
    )

    if !errorlevel!==0 (
        echo   [OK] Flyway migrations completed
    ) else (
        echo   [FAIL] Flyway migrations failed. Run manually: mvnw flyway:migrate
        goto :eof
    )
)
